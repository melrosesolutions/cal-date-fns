...
> **Status: Foundation + `parse/`, `format/`, `arithmetic/`, and `difference/` implemented.** `types/`, `errors/`, `internal/`, `epoch/` are complete. `parse/` now includes `parse.ts` and the full format-string parser, alongside `toObject`, `isValid`, `isCalDate`, `isYearMonth`, `toCalDate`, and `toYearMonth`. `format/` now includes `format()` and its tests. `arithmetic/` currently includes `add` and `subtract`, and `difference/` currently includes `since` and `until`.
